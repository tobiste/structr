...
The Frechet (geodesic \eqn{L^2}) mean finds iteratively the vector that minimizes the distances to all other points.
...
### Hypothesis testing
To test if a line (here `line_NULL`) is the population mean of a given set of lineations, we need to calculate the confidence area of our population (here `lines`).
```{r test0}
line_NULL <- Line(70, 0)
```
...
To visualize the confidence area of our lines:
```{r test2}
plot(lines, col = 'grey')
...
We rejected the Null Hypothesis that the given line represents the population mean if the p-value is smaller than 5% for 95% confidence.
