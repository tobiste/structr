...
left-hand rule is followed. Angle ranges from 0 to 90 °. Use `sense`
...
One advantage of `"strabo"` objects is that the orientation data is accompanied with 
metadata such as GPS coordinates, descriptions etc. This makes it a data base which
can be filtered, arranged and manipulated using queries the way you want it.
...
```{r sort, echo=FALSE}
sort_by(strabo_prj, strabo_prj$data$quality)
```
...
Orientations in drill-cores are usually given by &alpha; and &beta; angles
(lineations on a plane additionally have a &gamma; angle) which describe
