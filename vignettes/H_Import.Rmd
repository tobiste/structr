...
This imports the  tab-separated file as an `data.frame` object with the first column
representing the row names, and the two other columns the headers of the table. Say these measurements represent plane
...
measurements. Then use `as.Pair()` or `as.Fault()` to parse the object into {structr}.
...
`read_strabo_mobile()` provide import of `.xls` and any character-separated table
files (e.g. `.csv` or `.txt`).
