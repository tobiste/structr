...
Parametric testing means that we assume a specific underlying distribution of the data. 
Here, we assume that the lineations are drawn from a von Mises-Fisher distribution.
...
```{r test_param2}
#| fig.alt: Diagram showing the Fisher confidence region of a mean
plot(lines, col = "grey")
...
<!-- Using the **Fisher F-test**, We can now derive a p-value for the  -->
<!-- Null-Hypothesis that the samples are drawn from the same von Mises Fisher distribution. -->
<!-- The test statistic F is a non-negative measure of dissimilarity between -->
<!-- two datasets. The value is zero when the datasets are identical. -->

<!-- ```{r fisher-test} -->
<!-- fisher_ftest(lines, line_NULL) -->
<!-- ``` -->
<!-- **Interpretation**: Here, the test statistic (0.26) is dramatically lower than the critical value threshold (3.05), there is no statistically significant difference between the two samples. -->


### Non-parametric Testing 

Non-parametric testing does require assumptions on the distribution of the data.
This is commonly achieved using bootstrapping and permutation techniques.
...
of vectors. 
The test statistic T is a non-negative measure of dissimilarity between
two datasets. The value is zero when the datasets are identical.
