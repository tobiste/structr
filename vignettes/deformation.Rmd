...
# ISA vectors
axes_ISA <- S_eig$vectors |> as.Vec3() |> Line()

# Amounts of stretching
S_eig$values
```
> shortcuts: `instantaneous_stretching_axes(L)` and `instantaneous_stretching(L)`

The eigenvectors of the velocity gradient tensor describe the **flow apophyses**:

```{r vel_eigenL}
L_eig <- eigen(L)

# Flow apophyses
flow_vectors <- L_eig$vectors |> t() |> as.Vec3()
...
The angle &alpha; between the apophyses is directly related to how close to simple shear or pure shear the deformation is. &alpha; is zero for simple shear and 90&deg; for simple shear, and the **kinematic vorticity number** $W_k$ is given by $W_k = \cos\alpha$: 
...
The **vorticity vector** is the eigenvector corresponding to the intermediate eigenvalue of the velocity gradient tensor:
...
# flow apophyses
lines(flow_apophyses, col = c('grey40', 'grey60'), lty = 2)
...
# cooridiniate axes of def-tensor
points(axes, pch = 15)
text(axes, labels = c('x', 'y', 'z'), pos = 1, font = 3)

# legend
legend(0, -1.1, xjust = 0.5, 
